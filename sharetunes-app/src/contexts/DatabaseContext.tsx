        ...
        usersRef.where('id', '!=', '').onSnapshot((querySnapshot) => {
            ...
            postsRef.where('id', '!=', '').onSnapshot((querySnapshot) => {
                ...
                postsIds.forEach((postId) => {
                    ...
                    postsFromDatabase[postId].usernameOfPublisher =
                        publisher.username;
                    postsFromDatabase[postId].emailOfPublisher =
                        publisher.email;
                    postsFromDatabase[postId].profilePictureOfPublisher =
                        publisher.profilePictureURL;

                    let postComments: Array<Comment> = [];
                    postsFromDatabase[postId].comments.forEach(
                        (comment: any) => {
                            let newComment = { ...comment };
                            let commentPublisherId = newComment.publisherId;
                            let commentPublisher =
                                usersFromDatabase[commentPublisherId];
                            newComment.emailOfPublisher =
                                commentPublisher.email;
                            newComment.usernameOfPublisher =
                                commentPublisher.username;
